    ...
    <div className="min-h-svh flex flex-col items-center justify-center px-12 py-10">
      ...
      {phase === 'betting' && (
        <div className="text-center">
          ...
          <p className="text-white/50 text-2xl mb-8">{Object.keys(state.bets).length}/{state.order.length} ont misé</p>
          <div className="flex flex-wrap gap-3 justify-center max-w-4xl">
            {participants.map((m) => (
              <div key={m.id} className="glass-card rounded-2xl px-5 py-3 bg-black/40 flex items-center gap-2">
                ...
                <span className="text-white/50">{state.bets[m.id] !== undefined ? `✅ ${state.bets[m.id]}` : '…'}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {(phase === 'playing' || phase === 'results') && (
        <div className="w-full max-w-6xl">
          ...
          <div className="text-center mb-8">
            <p className="text-white/40 uppercase tracking-widest text-lg mb-3">
            ...
            {participants.map((m) => {
              ...
              return (
                <motion.div
                  ...
                  className={`glass-card rounded-2xl px-4 py-3 bg-black/40 flex flex-col items-center gap-2 ${
                ...
                >
                  ...
                  {r && (
                    <span className={`text-sm font-bold ${won ? 'text-emerald-300' : r.outcome === 'push' ? 'text-white/60' : 'text-pink-300'}`}>
          ...
          <p className="text-center text-white/30 text-xl mt-8">
...
function IntroScreen({ adult }: { adult: boolean }) {
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="glass-card rounded-3xl px-12 py-10 max-w-2xl bg-black/40 text-center">
      ...
      <p className="text-white/40 text-xl mt-6">L'hôte distribue 📱</p>
    </motion.div>
  )
}
