    ...
    return (
      <PageTransition>
        <div className="px-6 pt-10 pb-6 safe-top text-center">
          <p className="text-xs uppercase tracking-widest text-white/40">Débats</p>
    ...
    <PageTransition>
      <div className="px-6 pt-10 pb-6 safe-top">
        <div className="text-center mb-6">
          <p className="text-xs uppercase tracking-widest text-white/40">Débats du groupe</p>
          <h1 className="text-3xl font-extrabold shimmer-text mb-1">À vous de jouer</h1>
          <p className="text-white/50 text-sm">Des sujets générés à partir de vos plus grandes différences</p>
        </div>
              ...
              <Card key={d.id} delay={0.05 * i}>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex flex-col items-center gap-1.5 flex-1">
                    ...
                    <p className="text-[11px] text-white/50 text-center">défend {d.poleA}</p>
                  </div>
                  <span className="text-white/30 font-bold text-sm px-2">VS</span>
                  <div className="flex flex-col items-center gap-1.5 flex-1">
                    <Avatar pseudo={d.memberB.pseudo} color={d.memberB.color} size={44} />
                    <p className="text-sm font-semibold">{d.memberB.pseudo}</p>
                    <p className="text-[11px] text-white/50 text-center">défend {d.poleB}</p>
                  </div>
                </div>
              </Card>
            ))}
