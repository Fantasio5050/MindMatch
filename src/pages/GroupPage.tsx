    ...
    <PageTransition>
      <div className="px-6 pt-10 pb-6 safe-top">
        <div className="text-center mb-6">
          <p className="text-xs uppercase tracking-widest text-chalk-faint">Groupe</p>
        ...
        {finished.length < 2 ? (
          <Card className="text-center">
            ...
            <p className="text-chalk-soft text-sm mb-4">
              Partage le code ci-dessous pour qu'ils rejoignent le groupe et répondent au questionnaire.
            </p>
            <p className="text-2xl font-bold tracking-[0.3em] bg-felt-raised border border-line rounded-2xl py-3">
              {group.code}
            </p>
            {pending.length > 0 && (
              <div className="mt-4 flex flex-wrap gap-2 justify-center">
                {pending.map((m) => (
                  <span key={m.id} className="text-xs px-3 py-1.5 rounded-full bg-felt-raised text-chalk-soft">
                    {m.pseudo} en attente…
                  </span>
                ))}
              </div>
...
function GroupInsights({ members }: { members: ReturnType<typeof finishedMembers> }) {
  ...
  return (
    <div className="flex flex-col gap-4">
      <Card delay={0.05}>
        <h3 className="text-sm font-bold text-chalk-muted mb-3">Toutes les personnalités</h3>
        <RadarChart
          size={260}
      ...
      <Card delay={0.1}>
        <h3 className="text-sm font-bold text-chalk-muted mb-3">🤝 Vos points communs</h3>
        <div className="flex flex-col gap-3">
          {similarities.map((s) => (
            <div key={s.trait} className="flex items-center gap-3 rounded-2xl bg-felt-raised border border-line px-4 py-3">
              <span className="text-xl">{TRAIT_MAP[s.trait].emoji}</span>
              <div>
                <p className="text-sm font-semibold">{traitLabel(s.trait)}</p>
                <p className="text-xs text-chalk-soft">Vous êtes tous alignés, autour de {Math.round(s.avg)}/100</p>
              </div>
            </div>
          ))}
        </div>
      ...
      {differences.length > 0 && (
        <Card delay={0.15}>
          <h3 className="text-sm font-bold text-chalk-muted mb-3">⚡ Vos plus grandes différences</h3>
          <div className="flex flex-col gap-3">
            {differences.map((d) => (
              <div key={d.trait} className="rounded-2xl bg-felt-raised border border-line px-4 py-3">
                <div className="flex items-center gap-2 mb-2">
                  <span className="text-xl">{TRAIT_MAP[d.trait].emoji}</span>
                  <p className="text-sm font-semibold">{traitLabel(d.trait)}</p>
                </div>
                <div className="flex items-center justify-between text-xs text-chalk-soft">
      ...
      <Card delay={0.2}>
        <h3 className="text-sm font-bold text-chalk-muted mb-3">🏆 Classements amusants</h3>
        <div className="flex flex-col gap-2">
          {rankings.map((r) => (
            <div key={r.id} className="flex items-center gap-3 rounded-2xl bg-felt-raised border border-line px-3 py-2.5">
              ...
              <div className="flex-1">
                ...
                <p className="text-[11px] text-chalk-soft">{r.title}</p>
              </div>
              <span className="text-sm font-bold text-chalk-muted">{r.value}</span>
            </div>
          ))}
        </div>
      </Card>
      ...
      <Card delay={0.25}>
        <h3 className="text-sm font-bold text-chalk-muted mb-3">Membres</h3>
        <div className="flex flex-col gap-2">
          {members.map((m) => {
